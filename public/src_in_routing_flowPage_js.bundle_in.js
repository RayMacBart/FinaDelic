...
class FlowPage {
  ...
  #renderFlowPage(bagName, stepUp = false, toolbarReset = false) {
    ...
    if (!(Object.keys(bagData).length === 2 && 'IN' in bagData && 'OUT' in bagData)) {
      ...
      if (this.toolbar.currentType === 'flow' && document.querySelector(".flowlist").children.length < this.lastFlowCount || document.querySelector('.menu--account-remove').dataset.removalHappened === 'true') {
        toolbarReset = true;
      }
      if (toolbarReset) {
