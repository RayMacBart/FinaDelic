// IS NOT FULLY MIGRATED TO TS YET!

import { AppDataType } from "./appData";
import { ModalContentsType } from "./modalContents";
...
type ModalElements = {
   ...
   'input': HTMLElement;
   'start-date-label': HTMLElement;
   'start-date': HTMLElement;
   'end-date-label': HTMLElement;
   'end-date': HTMLElement;
   'select-label': HTMLElement;
   'select': HTMLElement;
   'amount-input-wrapper': HTMLElement;
   'submit-button': HTMLElement;
   'date-submit-button': HTMLElement;
   'cancel-button': HTMLElement;
}
...
export interface ModalType {
   ...
   smallInputLabelModalTypes: ['bag-create', 'bag-rename'];
   currentModalType?: ModalName;
   isModalSeries: boolean;
   appData: AppDataType;
   modalContents: ModalContentsType;
   dialog: HTMLElement;
   elements: ModalElements;

   boundSubmitFunction: () => void;  // annotate bound functions as properties (with arrow), not as methods!
   boundCancelFunction: () => void;
   inputModal: ...;  // SEPARATED INTERFACE FILE MUST BE CREATED AND HERE IMPORTED!
   selectModal: ...;  // SEPARATED INTERFACE FILE MUST BE CREATED AND HERE IMPORTED!
   modSub: ...;  // SEPARATED INTERFACE FILE MUST BE CREATED AND HERE IMPORTED!

   startModal(modalType: ModalName, isModalSeries: boolean): void;
   finishModal(): void;
   submitModal(): void;
   getAdjustedInnerText(elemName: string): string;
   runModal(): void;
}
   ...
   constructor(appData, modalContents) {
      ...
      this.inputModal = new InputModal(this);  // SEPARATED INTERFACE FILE FOR THIS MODULE MUST BE CREATED AND HERE IMPORTED!
      this.selectModal = new SelectModal(this);  // SEPARATED INTERFACE FILE FOR THIS MODULE MUST BE CREATED AND HERE IMPORTED!
      this.modSub = new ModalSubmitAllocator(this.appData);  // SEPARATED INTERFACE FILE FOR THIS MODULE MUST BE CREATED AND HERE IMPORTED!
